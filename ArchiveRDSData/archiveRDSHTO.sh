...
while [ "$current_start_date" -le "$end_date_epoch" ]; do
    ...
    vt_group_numbers=$(timeout $TIMEOUT_PERIOD time mysql -h"$DB_HOST" -u"$DB_USER" --port=$DB_PORT -p"$DB_PASSWORD" -D"$DB_NAME" -sN -e "SELECT distinct visitor_group_no FROM hotel_ticket_overview where last_modified_at BETWEEN '$start_date' and '$end_date'") || exit 1
    ...
    for ((i=0; i<$total_vt_groups; i+=BATCH_SIZE)); do
        ...
        if [ -z "$batch_str" ]; then
            ...
            querydata="select IFNULL(TRIM(TRAILING ',' FROM GROUP_CONCAT(DISTINCT(visitor_group_no))), '') as order_id from (SELECT visitor_group_no, max(last_modified_at) as mx_last_modified, min(last_modified_at) as mn_last_modified FROM hotel_ticket_overview WHERE visitor_group_no in ($batch_str) group by visitor_group_no having mn_last_modified < '2024-01-01 00:00:01' and mx_last_modified < '2024-01-01 00:00:01') as base"
