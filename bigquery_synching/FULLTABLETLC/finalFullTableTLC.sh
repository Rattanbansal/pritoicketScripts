...
echo "Final Full Table TLC Script is Running..........."
...
if [[ $UploadData == 2 ]]; then
  ...
  while :; do
    ...
    records=$(mysql -h $DBHOST --user=$DBUSER --port=$PORT --password=$DBPWD $DBDATABASE -N -e "select count(*) from (select * from ticket_level_commission where deleted = '0' and last_modified_at > '$previous_date'  limit $OFFSET, $LIMIT) as base;") || exit 1

    echo "$records"

    echo "select * from ticket_level_commission where deleted = '0' and last_modified_at > '$previous_date' limit $OFFSET, $LIMIT" | time mysqlsh --sql --json --uri $DBUSER@$DBHOST:$PORT -p$DBPWD --database=$DBDATABASE >> ticket_level_commission_primarypt.json || exit 1
