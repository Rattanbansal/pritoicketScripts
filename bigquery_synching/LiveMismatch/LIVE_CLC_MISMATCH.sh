...
if [[ $UploadData == 2 ]]; then
    ...
    RecordCount=$(timeout $TIMEOUT_PERIOD time mysql -h $DBHOST --user=$DBUSER --port=$PORT --password=$DBPWD $DBDATABASE -N -e "select count(*) from (select * from channel_level_commission where deleted = '0' and last_modified_at > '$previous_date' limit $offset, $limit) as base;") || exit 1
    ...
    echo "select $SELECTCOLUMNS from channel_level_commission where deleted = '0' and last_modified_at > '$previous_date' limit $offset, $limit" | timeout $TIMEOUT_PERIOD time mysqlsh --sql --json --uri $DBUSER@$DBHOST:$PORT -p$DBPWD --database=$DBDATABASE >> "$offset"_primarypt.json || exit 1
