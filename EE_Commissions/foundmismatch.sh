...
# select * from (select * from (select DISTINCT(ticket_id) as ordered_product from bigqueryData where reseller_id = '541') as orderData left join (select distinct(ticket_id) as ticket_id from distributors) d on orderData.ordered_product = d.ticket_id) as final where ticket_id is NULL; 
