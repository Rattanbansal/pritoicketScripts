...
ticket_ids=$(mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" -D"$DB_NAME" -sN -e "SELECT DISTINCT(ticketid) FROM orders")
...
for ticket_id in $ticket_ids; do
    ...
    for ((i=0; i<$total_vt_groups; i+=BATCH_SIZE)); do
        ...
        echo "------$(date '+%Y-%m-%d %H:%M:%S')--------" >> vt_update_query.sql

        echo "$update_query_vt" >> vt_update_query.sql

        

        update_query_pt="UPDATE
...
FROM
    visitor_tickets
WHERE
    vt_group_no in ($batch_str) and action_performed like '%CSSCommission'
GROUP BY
    vt_group_no,
    transaction_id,
  ...
  echo "------$(date '+%Y-%m-%d %H:%M:%S')--------" >> pt_update_query.sql
