...
sum(case when row_type =1 then partner_net_price else 0 end) as sale_price,sum(case when row_type =3 then partner_net_price else 0 end) as Distributor_price, max(last_modified_at) as last_modified_at from vt group by vt_group_no,transaction_id)

select *except(last_modified_at),round((Distributor_price/sale_price)*100,2) as commission, last_modified_at from main"
...
LINES TERMINATED BY '\n'
IGNORE 1 LINES
(vt_group_no, transaction_id, hotel_id, ticket_id, ticketpriceschedule_id, channel_id, reseller_id, sale_price,Distributor_price, commission, last_modified_at);
EOF
