...
from_date=$1
to_date=$2
OUTPUT_FILE="reports/bigqueryReportDistributorPercentage.csv"
...
case when tlcsaleprice is not null then tlchotelpercentage when tlcsaleprice is null and catalogsaleprice is not null then cataloghotelpercentage when tlcsaleprice is null and catalogsaleprice is null and clcsaleprice is not null then clchotelpercentage else 111 end as hotelpercentageshouldbe,
...
case when tlcsaleprice is not null then 'tlcsetting' when tlcsaleprice is null and catalogsaleprice is not null then 'catalogsetting' when tlcsaleprice is null and catalogsaleprice is null and clcsaleprice is not null then 'clcsetting' else 'Nosetting' end as pricinglevel from getpricestting) select *, cast(distributorcommission/saleprice*100 as float64) as percentageCommission, hotelpercentageshouldbe from mismatches where ABS(cast(distributorcommission/saleprice*100 as float64)-(hotelpercentageshouldbe)) > 0.03 and saleprice > 0.00 and saleprice is not NULL union all select *, cast(distributorcommission/saleprice*100 as float64) as percentageCommission, hotelpercentageshouldbe from mismatches where ABS(cast(distributorcommission as float64)-(hotelfeeshouldbe)) > 0.03 and saleprice = 0.00 and saleprice is NULL"
