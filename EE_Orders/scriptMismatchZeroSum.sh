...
timeout $TIMEOUT_PERIOD time mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" -D"$DB_NAME" -sN -e "update $tableName set channel_id = '0';select ROW_COUNT();" || exit 1  # channel_id used as status because not needed this column
...
for ticket_id in $ticket_ids; do
    ...
    vt_group_numbers=$(timeout $TIMEOUT_PERIOD time mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" -D"$DB_NAME" -sN -e "SELECT distinct vt_group_no FROM $tableName WHERE ticketId = '$ticket_id' and channel_id = '0'") || exit 1
    ...
    for ((i=0; i<$total_vt_groups; i+=BATCH_SIZE)); do
        ...
        timeout $TIMEOUT_PERIOD time mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" -D"$DB_NAME" -sN -e "update $tableName set channel_id = '3' where vt_group_no in ($batch_str)" || exit 1  # channel_id used as status because not needed this column
