...
for ticket_id in $ticket_ids; do
    ...
    for ((i=0; i<$total_vt_groups; i+=BATCH_SIZE)); do
        ...
        MISMATCH="SELECT IFNULL(TRIM(TRAILING ',' FROM GROUP_CONCAT(DISTINCT(final.vt_group_no))), '') as order_id
FROM
    (
    SELECT
        vt_group_no,
...
ON
    scdata.ticketpriceschedule_id = pl.ticketpriceschedule_id AND scdata.ticketId = pl.ticket_id AND scdata.channel_id = pl.channel_id AND pl.catalog_id = '0' AND pl.is_adjust_pricing = '1' AND pl.deleted = '0'
            ...
            timeout $TIMEOUT_PERIOD time mysql -h"$DB_HOST" -u"$DB_USER" -p"$DB_PASS" -D"$DB_NAME" -sN -e "update matrixReseller set status = '1' where visitor_group_no in ($batch_str);SELECT ROW_COUNT()" || exit 1
