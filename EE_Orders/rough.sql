...
(SELECT vt_group_no, targetlocation, ticketId, ticketpriceschedule_id FROM `visitor_tickets` where transaction_id = '171873357893317001' and col2 = 2 and action_performed like '%EECombiCommission')

SELECT visitor_group_no, prepaid_ticket_id,max(version) as version, clustering_id, ticket_id, tps_id FROM prepaid_tickets WHERE visitor_group_no = '171873357893317' and is_addon_ticket = '2' group by visitor_group_no, prepaid_ticket_id, clustering_id, ticket_id, tps_id 