...
JOIN(
    ...
    FROM
        visitor_tickets
    WHERE
        ticketId = '13928' and vt_group_no IN(171776704950379,172893838934417,172908159671277,173012903930237,171243484311137,172389335847147,173080614676304,173149253049043,172926744645777,172985344954719,171005534179743,171568367108962,171706740101861,172089040022971,172347905652159,172550603940993,172650947687195,172739677357910,172780321632004,172804521414679,172808058979404,172927220507340,172951760653302,172954275614306,172961785476325) AND col2 != '2' and transaction_type_name not like '%Reprice Surcharge%' and transaction_type_name not like '%Reprice Discount%'
    GROUP BY
        vt_group_no,
        transaction_id
) AS maxversion
